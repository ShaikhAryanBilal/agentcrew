...
without searching. Regenerate with: `.agentcrew\scripts\generate-codebase-map.ps1`

**2. Custom Instructions** â€” Before running any step, check `.agentcrew/custom/<phase>.md`.
If file has content, inject as additional context. Empty file = skip.

| Phase | Custom File |
|-------|-------------|
| Requirements | `.agentcrew/custom/01-requirements.md` |
| Design | `.agentcrew/custom/02-design.md` |
| Development | `.agentcrew/custom/03-development.md` |
| QA | `.agentcrew/custom/04-qa.md` |
| Deployment | `.agentcrew/custom/05-deployment.md` |
| Maintenance | `.agentcrew/custom/06-maintenance.md` |

**3. Logging** â€” Each step writes a log to `.agentcrew/log/<year>/<month>/<day>/<index>-<stepId>-<timestamp>.md` (WordPress-style date hierarchy). Use `.agentcrew/scripts/write-workflow-log.ps1` (or `write-workflow-log.sh`) to write logs.
State is tracked in `.agentcrew/state/workflow.json` (current step, completed steps with
timestamps, phase gates). On resume, agent reads state + last log for context.

**4. Validation** â€” Run `.agentcrew/scripts/validate-workflow.ps1` (or `.sh`) to check cross-references,
...
For small projects / MVPs, use `.agentcrew/light/00-router.md` (7-step light variant).
