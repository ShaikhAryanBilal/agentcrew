...
.EXAMPLE
  .\.crewmarkdown\animations\show-party.ps1 -Objective "Design Solution" -Squad PM,Architect,Backend,Security

.EXAMPLE
  .\.crewmarkdown\animations\show-party.ps1 -Objective "Build Feature" -Squad Frontend,Backend,QA -Timeout 5
#>
...
$ePM = '??'; $eBA = '??'; $eArch = '???'; $eTL = '?????'; $eFE = '??'
$eBE = '??'; $eMB = '??'; $eUX = '??'; $eDE = '???'; $eML = '??'
$eQA = '??'; $eSec = '???'; $eDO = '??'; $eEM = '??'; $eDB = '??'; $eMF = '??'; $eOrch = '??'
...
$asciiArtPool = @(
  ...
  @{ title = '?? SQUAD OF THE REALM  ';   desc = 'rpg sword' }
