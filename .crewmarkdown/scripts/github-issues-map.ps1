...
test → qa/plan
design → design/arch
epic → 00-objectives.md (manual routing)
...
.PARAMETER OutputDir
Where to write the mapping report (default: .crewmarkdown/logs)
#>
...
if (-not $OutputDir) {
  ...
  $OutputDir = Join-Path $ProjectRoot ".crewmarkdown" "logs"
}
...
$content = @"
# GitHub Issues → CrewMarkdown Map
